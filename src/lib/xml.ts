...
export function formatXml(input: string, mode: XmlMode = 'format', indent = '2') {
  ...
  if (/<!\s*(?:DOCTYPE|ENTITY)\b/i.test(input))
    throw new Error(
      'DTD and entity declarations are disabled. External resources are never resolved.',
    )
  ...
  const error =
    doc.getElementsByTagNameNS(
      ...
      'parsererror',
    )[0] ?? doc.getElementsByTagNameNS('http://www.w3.org/1999/xhtml', 'parsererror')[0]
  ...
  function render(node: Node, depth: number, inherited = false): string {
    ...
    const preserve =
      ...
      (inherited && element.getAttribute('xml:space') !== 'default')
    if (
      preserve ||
      children.some(
    ...
    const body = content
      .map((child) => (pretty ? unit.repeat(depth + 1) : '') + render(child, depth + 1, preserve))
      .join(pretty ? '\n' : '')
  ...
  return [
    ...
    ...Array.from(doc.childNodes)
      .filter((node) => node.nodeType !== 3 && !(node.nodeType === 7 && node.nodeName === 'xml'))
