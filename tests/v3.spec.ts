...
test('Workspace imports, duplicates, pins, previews, filtering, deletion, and confirmed clear', async ({
  ...
  await page.getByRole('button', { name: 'Pin sample.png', exact: true }).click()
  await page.reload()
  await expect(page.locator('.workspace-file').first()).toContainText('★ sample.png')
  await page.getByRole('button', { name: 'Copy filename sample.png', exact: true }).click()
  ...
  await page.keyboard.press('Escape')
  await page.goto('/workspace')
  await page.getByRole('button', { name: 'Delete sample-2.png', exact: true }).click()
...
test('new registry tools integrate with favorites, recent tools and command navigation', async ({
  ...
  await page.getByRole('combobox').fill('image compressor')
  await page.keyboard.press('Enter')
  await expect(page).toHaveURL('/tools/image-compressor')
  await page.keyboard.press('Control+k')
  await page.getByRole('combobox').fill('workspace')
  await page.keyboard.press('Enter')
